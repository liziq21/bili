...
| 独立 Trending | 不作为实现依据 | 已确认旧 `FEtrending` 不稳定/退役，不注册 | [InnerTube 使用说明](https://github.com/tombulled/innertube) | 不适用 | 已核对 |
...
- InnerTube client context、API key（如需要）、Cookie、Token 和追踪字段不得写入 fixture、日志或测试输出。

## 抓取与测试

- 真实抓取脚本放在 `tool/capture/`，从 `packages/youtube/ypi/` 目录手动运行；输出到 `testing/<endpoint>.json`。
- fixture 只保存原始 HTTP response body，不保存 headers、Cookie、Token、带凭据 URL 或追踪凭据；HTTP status 和请求元数据记录在 `testing/README.md`。
